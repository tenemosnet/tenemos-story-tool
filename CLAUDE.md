...
テネモスネットのLINE配信・メール通信向けコンテンツ生成ツール（v6.2）。Claude APIでストーリーを自動生成し、HP・ブログから収集したナレッジとユーザーフィードバックで品質を継続的に向上させる。生成したストーリーからメール通信原稿・WordPress記事原稿への変換、LINE配信用文書分割、配信スケジュール管理、ネタストック機能も備える。
...
npm run ingest:mail      # メール原稿をknowledge_sourcesに取り込み
...
1. ユーザーが `/stories` でLINE配信用分割ボタンをクリック
2. `LineSplitModal` で分割数（2/3/4）・スタイル（subtle/explicit/none）・追加指示を入力
3. `/api/split-line` POST で分割実行
   - `lib/converters/line-split.ts` の `splitStoryForLine()` を呼び出し
   - ストーリー本文のみ使用（ナレッジ参照なし）、max_tokens: 2048
   - JSON配列パース失敗時リトライ1回
4. 結果を `line_distributions` にINSERT、生成履歴を `line_distribution_history` に保存
5. `/line-distributions/[id]` へ遷移してプレビュー表示
6. 各回の配信予定日・完了フラグは PATCH で配列全体を更新
7. 再分割時は `parent_history_id` を指定し、同レコードをUPDATE（新履歴をINSERT）

### LINE配信CSV取込の仕組み

1. カレンダー画面でチャットブースト配信履歴CSVをドラッグ&ドロップ（またはクリックでファイル選択）
2. `/api/finished-contents/import-csv` へアップロード
3. `lib/importers/chatboost-line-csv.ts` でShift-JIS→UTF-8変換・RFC4180パース
4. 配信日で`finished_contents`（`type='line'`）を検索し、タイトル反映＋`is_done`更新
5. マッチしない配信は新規レコード作成、既存IDは重複スキップ
6. 配信統計（送信数・開封率等）は`chatboost_delivery_stats`テーブルに保存
7. カレンダー画面を再取得して結果モーダルを表示

- 反映するのは「配信日・タイトル・is_done」のみ。**本文（body）は反映しない**
- `chatboost_delivery_id`で重複防止。同一CSVの再取込は全件スキップ
- UPDATE時は`line_delivery_type`を保持（既存のbroadcast/segment設定を壊さない）
...
| `lib/types/line-distribution.ts` | LINE配信分割関連型定義（SplittingStyle, LineDistributionPart等） |
| `lib/converters/line-split.ts` | LINE配信分割ロジック（splitStoryForLine） |
| `app/api/split-line/route.ts` | LINE配信分割実行API（初回・再分割共通） |
| `app/api/line-distributions/route.ts` | LINE配信シリーズ一覧API |
| `app/api/line-distributions/[id]/route.ts` | LINE配信シリーズ単体取得・更新・削除API |
| `app/api/line-distributions/[id]/history/route.ts` | LINE配信分割履歴取得API |
| `app/line-distributions/[id]/page.tsx` | LINE配信分割プレビュー画面（各回コピー・日付・完了管理・再分割・履歴） |
| `components/line-split-modal.tsx` | LINE配信分割設定モーダル（分割数・スタイル選択） |
| `scripts/ingest/hp.ts` | HPスクレイピング（EUC-JP対応、Colormeオブジェクト解析） |
| `scripts/ingest/blog.ts` | ブログ収集 + Claude APIトーン分析 |
| `app/api/finished-contents/import-csv/route.ts` | LINE配信CSV取込API |
| `lib/importers/chatboost-line-csv.ts` | チャットブーストCSVパーサー（Shift-JIS対応） |

## データベース（Supabase）

16テーブル: `stories`, `templates`, `products`, `knowledge_sources`, `generation_logs`, `ingest_logs`, `stock_ideas`, `finished_contents`, `task_memos`, `output_templates`, `blog_stocks`, `blog_generation_history`, `line_distributions`, `line_distribution_history`, `app_settings`, `chatboost_delivery_stats`

- `blog_generation_history` はマイグレーション追加テーブル（`blog_stock_id` / `story_id` 参照、`parent_history_id` で再生成の系譜管理）
- `app_settings` はアプリ全体設定用テーブル（schema.sql定義）
...
- `finished_contents`は配信予定コンテンツ（LINE/メール種別、日付・完了管理）。`chatboost_delivery_id`でCSV取込済みレコードを重複防止
- `chatboost_delivery_stats`はチャットブーストCSVの配信統計ログ（送信数・開封率等、将来の分析用）
